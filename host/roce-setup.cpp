...
enum RoceArg {
	ARG_RPSN = 0, ARG_LPSN, ARG_RQPN, ARG_LQPN,
	ARG_RIP, ARG_LIP, ARG_RUDP, ARG_VADDR, ARG_RKEY,
	ARG_OP, ARG_RADDR, ARG_LADDR, ARG_LEN,
	ARG_DEBUG, ARG_MEM_PTR
};
...
bool configure_roce(
	...
	if (err != CL_SUCCESS) {
		std::cout << "No rocetest_krnl in xclbin (emulation build?); "
		             "skipping RoCE setup" << std::endl;
		return false;
	}

	const NodeConfig *me = nullptr, *peer = nullptr;
	for (auto& nc : nodes) {
		if (nc.id == my_id) me = &nc;
		else peer = &nc; // two-node first light: single peer
	}
	...
	OCL_CHECK(err, err = k.setArg(ARG_DEBUG, (uint32_t)0));
