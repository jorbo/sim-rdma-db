...
grep -F "+ rm -f build_dir.hw.$platform.server0/krnl.xclbin" <<<"$full" >/dev/null
grep -F "+ make build TARGET=hw SERVER=0 PLATFORM=$platform" <<<"$full" >/dev/null
grep -F "+ rm -f build_dir.hw.$platform.server1/krnl.xclbin" <<<"$full" >/dev/null
...
grep -F "+ rm -f build_dir.hw.$platform.server1/krnl.xclbin" <<<"$server1" >/dev/null
